...
class Parcel:
    ...
    def __str__(self):
        total_size = self.height + self.width + self.length
        return (f"Size: {total_size:.1f} cm (H+W+L), "
...
class ParcelOrderApp:
    ...
    def create_layout(self):
        ...
        tk.Button(frame, text="Remove Last Parcel", command=self.remove_last_parcel).grid(row=11, column=0, columnspan=2, pady=5)
        tk.Button(frame, text="Finish Order", command=self.display_receipt).grid(row=12, column=0, columnspan=2, pady=10)
    ...
    def add_parcel(self):
        ...
        try:
            ...
            if height <= 0 or width <= 0 or length <= 0 or weight <= 0:
                messagebox.showerror("Error", "All dimensions and weight must be positive numbers.")
                return
    ...
    def display_receipt(self):
        ...
        if not name or not address or not phone.isdigit() or len(phone) not in [10, 11]:
